...
export function VolunteerSectionBuilder() {
	...
	const handleAdd = () => {
		openDialog("resume.sections.volunteer.create", undefined);
	};

	const buildSubtitle = (item: z.infer<typeof volunteerItemSchema>) => {
		const parts = [item.location, item.period].filter((part) => part && part.trim().length > 0);
