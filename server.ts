 ...
 * @see https://github.com/tanstack/router/blob/main/examples/react/start-bun/server.ts
 *
 ...
 *   - Example: ASSET_PRELOAD_MAX_SIZE=10_485_760 (10MB)
 *
...
const MAX_PRELOAD_BYTES = Number(
	process.env.ASSET_PRELOAD_MAX_SIZE ?? 10 * 1024 * 1024, // 10MB default
);
...
await migrateDatabase().catch((error: unknown) => {
	log.error(`Failed to migrate database: ${String(error)}`);
	process.exit(1);
});
